...
export default function BlogPostCard({ item }: { item: blogPostType }) {
  return (
    <div
      key={item.id}
      className='mx-4 flex flex-col justify-around rounded-md bg-secondary-color transition-all duration-500 ease-in-out hover:bg-[#132444] sm:flex-row xl:mx-0  overflow-hidden border-2 border-secondary-color hover:border-[#1d3666]'
    >
      {/* <CloudImage
        src={item.imgUrl}
        alt={item.title}
      ...
      <Image
        ...
        height={500}
        width={500}
        className='h-72 w-[550px] rounded-t-md object-cover opacity-95 transition-opacity duration-300 ease-in-out hover:opacity-100 sm:rounded-s-md sm:rounded-tr-none md:h-80   '
      />
