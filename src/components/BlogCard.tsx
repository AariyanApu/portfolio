...
export default function BlogCard({ item }: { item: blogPostType }) {
  ...
  return (
    <div
      key={item.id}
      className="mx-4 flex flex-col justify-start gap-x-16 overflow-hidden rounded-md border border-secondary-color bg-secondary-color transition-all duration-500 ease-in-out  hover:border-[#1d3666] hover:bg-[#132444] lg:flex-row xl:mx-0"
    >
      <CloudImage
        src={item.imgUrl}
        alt={item.title}
      ...
      />
      <div className=" my-2 flex flex-col gap-y-2 px-4 pb-1 pl-4 sm:py-6 sm:pl-8 ">
